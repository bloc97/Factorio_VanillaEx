...
ExNature={
  ...
  setTide = function(tide)
    ExNature.tide = tide
  end;

  updateTide = function(gametick)
    ...
    if (gametick%(tide_period_moon*2)) <= (tide_period_moon) then -- Checks whether it is first or second tide.
      ExNature.tide = ExFmath.zero((tides_moon - tides_sun))/1.6
    else
      ExNature.tide = ExFmath.zero((tides_moon - tides_sun))
    end
  ...
  setWindGoal = function(windgoal)
    ExNature.windgoal = windgoal
  end;
  ...
  updateWindFluc = function()
    if not ExNature.iswindgoal then
      local offset = math.random(-100,100)*0.0001
      ExNature.addWind(offset)

    else
      if not ExFmath.checkdiff(ExNature.wind,ExNature.windgoal,0.05) then ExNature.iswindgoal=false
      elseif ExNature.wind>ExNature.windgoal then ExNature.addWind(-0.02*math.random())
      elseif ExNature.wind<ExNature.windgoal then ExNature.addWind(0.02*math.random())
      else ExNature.iswindgoal=false
      end
    end
  end;
