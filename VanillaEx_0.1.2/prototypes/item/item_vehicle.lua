...
item_vehicle[2]=util.table.deepcopy(data.raw["item-with-entity-data"]["tank"])
item_vehicle[2].name="tank-arty"
item_vehicle[2].order="b[personal-transport]-b[tank]-d"

item_vehicle[3]=util.table.deepcopy(data.raw["item-with-entity-data"]["tank"])
item_vehicle[3].name="tank-auto"
item_vehicle[3].order="b[personal-transport]-b[tank]-b"

item_vehicle[4]=util.table.deepcopy(data.raw["item-with-entity-data"]["tank"])
item_vehicle[4].name="car-armour"
item_vehicle[4].order="b[personal-transport]-a[car]-a"

item_vehicle[5]=util.table.deepcopy(data.raw["item-with-entity-data"]["tank"])
item_vehicle[5].name="car-truck"
item_vehicle[5].order="b[personal-transport]-a[car]-b"

item_vehicle[6]=util.table.deepcopy(data.raw["item-with-entity-data"]["tank"])
...
for i=2,item_vehicle[0] do -- Generates icons and place_result using name
item_vehicle[i].icon=toicon(item_vehicle[i].name)
item_vehicle[i].place_result=item_vehicle[i].name
end

for e=2,item_vehicle[0] do -- Extends Factorio table (adds item to the game)
data:extend({
item_vehicle[e],
})
end
